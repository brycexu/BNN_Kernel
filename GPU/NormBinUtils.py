...
class BinarizeConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding):
        ...
        self.weight = nn.Parameter(
            torch.FloatTensor(out_channels, 1 + (in_channels * self.kernel_size[0] * self.kernel_size[1] - 1)), requires_grad=False)
...
def BinLinFunction(input, weight, bias):
    ...
    binop.binary_gemm(input, weight, output, m, n, k)
...
class BinarizeLinear(nn.Linear):
    def __init__(self, in_channels, out_channels):
        ...
        # self.weight = nn.Parameter(torch.FloatTensor(out_channels, 1 + (in_channels - 1)), requires_grad=False)

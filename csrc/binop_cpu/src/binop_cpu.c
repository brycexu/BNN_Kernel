...
void binary_gemm_cpu(THIntTensor* a, THIntTensor* b, THFloatTensor* c, int m, int nn, int k) {
    ...
    int n = 1 + (nn-1) / ENCODE_BIT;
    dgemm_nn(m, k, nn, A, n, 1, B, k, 1, C, k, 1);
}
...
void encode_rows_cpu(THIntTensor* input, THIntTensor* output) {
    ...
    uint32_t* a = (uint32_t*)THIntTensor_data(input);
...
void encode_cols_cpu(THFloatTensor* input, THIntTensor* output) {
    ...
    uint32_t* a = (uint32_t*)THFloatTensor_data(input);
    uint32_t* b = (uint32_t*)THIntTensor_data(output);
...
static void BinaryConvolution_cpu_helper(
    THFloatTensor *output, THIntTensor *weight, THFloatTensor *bias, THFloatTensor *ones, THIntTensor *bin_col,
    int kW, int kH, int dW, int dH, int padW, int padH,
    ...
    binary_gemm_cpu(weight, bin_col, output2d, nOutputPlane, kW*kH*nInputPlane, outputHeight*outputWidth);
    if (bias->nDimension) {
...
void BinaryConvolution_cpu(
    ...
    THFloatTensor *columns = THFloatTensor_new();
...
#pragma omp parallel for private(t)
    ...
    for(t = 0; t < T; t++){
        ...
        BinaryConvolution_cpu_helper(
            output_t, weight, bias, ones, bin_col_t, kW, kH, dW, dH, padW, padH,

...
void binary_gemm(THCudaIntTensor* a, THCudaIntTensor* b, THCudaTensor* c, int m, int nn, int k){
    ...
    binary_gemm_cuda(A, B, C, m, nn, k, stream);
}

void gemm(THCudaTensor* a, THCudaTensor* b, THCudaTensor* c, int m, int n, int k) {
    if (c->nDimension != 2 || c->size[0]*c->size[1] < m*k) {
        THCudaTensor_resize2d(state, c, m, k);
    }
    float* A = THCudaTensor_data(state, a);
    float* B = THCudaTensor_data(state, b);
    float* C = THCudaTensor_data(state, c);
    cudaStream_t stream = THCState_getCurrentStream(state);
...
void BinaryConvolution(
    ...
    for (int elt = 0; elt < batchSize; elt ++) {
		...
		gemm(weight, columns, output_n, m, n, k);
